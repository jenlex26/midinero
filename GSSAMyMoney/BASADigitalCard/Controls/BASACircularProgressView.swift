...
//
//  Created by BranchbitG on 17/05/21.
//
...
@IBDesignable class BASACircularProgressView: UIView {
    //Delegate
    ...
    public func start(beginingValue: Int, interval: TimeInterval = 1) {
        ...
        timer?.invalidate()
        //Re-initialize TImer
        timer = Timer(timeInterval: fireInterval, target: self, selector: #selector(BASACircularProgressView.timerFired(_:)), userInfo: nil, repeats: true)
